...
export default class VideoComponent extends Component {
  ...
  constructor(video: Video, subComp: SubComponent) {
    ...
    const thumbLargeItem = $("<input/>", { "class": "ytbsp-thumb-large-url", "type": "hidden", "value": this.video.thumbLarge ? this.video.thumbLarge : this.video.thumb });
    const titleItem = $("<a/>", { "href": `/watch?v=${this.video.id}`, "class": "ytbsp-title", "title": this.video.title, "html": this.video.title });
    ...
    this.seenMarkerItem = $("<p/>", { "class": `ytbsp-seenMarker${this.video.seen ? " seen" : ""}`, "html": (this.video.seen ? "already seen" : "mark as seen") });
